...
void WalkingController::Joyori(){
    ...
    double delta_theta = 10*DEG2RAD;
    unsigned int number_of_footstep = 5;

    double width = 0.127794;
    double dlength = 0.1; // for stride. .
...
//               foot_step_joy_(1,6) = 0.5+0.5*temp;
       ...
       if(current_step_num_<=2){
           ...
           for(int i=0;i<remain;i++){
               if(theta_input<0)
                   foot_step_joy_(i,5) = -(i+1)*delta_theta;
               else {
                   foot_step_joy_(i,5) = (i+1)*delta_theta;
               }
           }
...
//           }
           ...
           for(int i=0;i<remain;i++){
               if(theta_input<0){
                  foot_step_joy_(i+2,5) = foot_step_joy_(1,5) - (i+1)*delta_theta;
               }
               else {
                  foot_step_joy_(i+2,5) = foot_step_joy_(1,5) + (i+1)*delta_theta;
               }
...
void WalkingController::getJoystick(double x, double y, double z)
{
