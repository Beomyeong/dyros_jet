...
static void  ComplimantaryFilter(Eigen::Vector3d accData,
                                            ...
                                            )
{
   ...
   // angle(0) +=((double)gyrData(0) / GYROSCOPE_SENSITIVITY) / hz; //angle around x-axis
    //angle(1) +=((double)gyrData(1) / GYROSCOPE_SENSITIVITY) / hz; //angle around y-axis
...
//    {
//        //Turning aound X axis results in a vector on the Y-axis
//        rollAcc = atan2((double)accData(1),(double)accData(2)) ; // *180/PI make to degree
//        angle(0) = angle(0)*0.98 + rollAcc*0.02;

//        //Turning around Y acis results in a cvector on the x-axis
//        pitchAcc = atan2((double)accData(0),(double)accData(2));//*180/PI make to degree
//        angle(1) = angle(1) * 0.98 + pitchAcc * 0.02;
//    }

    double cutoff_freq = 0.1;
    float wn = 2.0*3.1415*cutoff_freq;
    float alpha_const = (1.0/wn*hz)/(1.0+1.0/wn * hz);


    angle(0) += (double)gyrData(0) /hz;
    angle(1) += (double)gyrData(1) /hz;

    rollAcc = atan2((double)accData(1),(double)accData(2));
    //angle(0) = 0.99*angle(0) + 0.01*rollAcc;
    angle(0) = alpha_const*angle(0) + (1-alpha_const)*rollAcc;

    pitchAcc = atan2(-(double)accData(0), (double)accData(2));
    //angle(1) = 0.98*angle(1) + 0.02*pitchAcc;
    angle(1) = alpha_const*angle(1) + (1-alpha_const)*pitchAcc;
...
static double cubic2Dot(double time,     ///< Current time
             double time_0,   ///< Start time
             double time_f,   ///< End time
             double x_0,      ///< Start state
             double x_f,      ///< End state
...
static Eigen::Matrix3d rotateDotWithY(double pitch_anlge)
{
    Eigen::Matrix3d rotate_dot_with_y;

    rotate_dot_with_y.setZero();

    rotate_dot_with_y(0,0) = -sin(pitch_anlge);
    rotate_dot_with_y(0,2) = cos(pitch_anlge);

    rotate_dot_with_y(2,0) = -cos(pitch_anlge);
    rotate_dot_with_y(2,2) = -sin(pitch_anlge);

    return rotate_dot_with_y;
}
...
static Eigen::MatrixXd LowerTriangularMatrix(Eigen::MatrixXd matrix){
...
static Eigen::MatrixXd UpperTriangularMatrix(Eigen::MatrixXd matrix){
    int rows = matrix.rows();
    int cols = matrix.cols();
